...
int existsFifoServidor_C1(char* nameFifo) {
    ...
    if (stat(nameFifo, &st) == 0) {
        if (S_ISFIFO(st.st_mode)) {
            result = 0;
            so_success("C1", "");
        } else so_error("C1", "File is not a FIFO");
    } else
        so_error("C1", "Could not get stat for file %s", nameFifo);
...
int triggerSignals_C2() {
    ...
    result =
        signal(SIGUSR2, trataSinalSIGUSR2_C12) == SIG_ERR ||
        signal(SIGINT, trataSinalSIGINT_C13) == SIG_ERR ||
...
Login getDadosPedidoUtilizador_C3() {
    ...
    printf("Introduza os seus dados de login\n");
    char s[20];

    printf("NIF: ");
    if (!so_gets(s, 10)) { // NIF is 9 digits long + \0
        so_error("C3", "Failed to read NIF");
        return request;
    }
    request.nif = atoi(s);

    printf("Senha: ");
    if (!so_gets(s, sizeof(request.senha))) { // Should return the pointer to the string or null
        so_error("C3", "Failed to read password");
        request.nif = -1;
        return request;
    }
    strcpy(request.senha, s);
...
int createFifo_C4(char* nameFifo) {
    int result = -1;    // Por omissão retorna erro
    so_debug("< [@param nameFifo:%s]", nameFifo);
    if (buildNomeFifo(nameFifo, SIZE_FILENAME, FILE_PREFIX_CLIENT, getpid(), FILE_SUFFIX_FIFO) == -1) {
        so_error("C4", "Failed to get FIFO name");
        return -1;
    }
    ...
    if (result == 0)
        ...
        so_error("C4", "Could not create file %s", nameFifo);
...
int writeRequest_C5(Login request, char* nameFifo) {
    ...
    if (!f) {
        so_error("C5", "Failed to open fifo %s", nameFifo);
        deleteFifoAndExit_C11();
    } else {
        if (fwrite(&request, sizeof(Login), 1, f) == 1) {
            result = 0;
            so_success("C5", "");
        } else {
            so_error("C5", "Failed to write to fifo");
        }
        fclose(f);
        if (result == -1) deleteFifoAndExit_C11();
    }
...
Login readAckLogin_C7(char* nameFifo) {
    ...
    if (!f) {
        so_error("C7", "Failed to open file %s", nameFifo);
        deleteFifoAndExit_C11();
    } else {
        if (fread(&ackLogin, sizeof(ackLogin), 1, f) < 1) { // If it reads less than 1 element
            so_error("C7", "Failed to read from file %s", nameFifo);
            fclose(f);
            deleteFifoAndExit_C11();
        }
        fclose(f);
...
/**
 * @brief C8    “Desliga” o alarme configurado em C6.
...
int writeFimSessao_C10(char* nameFifo, int tempoDeProcessamento, Login ackLogin) {
    ...
    FILE* f = fopen(nameFifo, "w");
    if (f) {
        if (fprintf(f, "Sessão Login ativa durante %d segundos", tempoDeProcessamento) < 0) {
            so_error("C10", "Failed to read from file %s", nameFifo);
        } else {
            result = 0;
            so_success("C10", "");
        }
        ...
        so_error("C10", "Failed to open file %s", nameFifo);
...
void deleteFifoAndExit_C11() {
    ...
    if (buildNomeFifo(nomeFifoCliente, SIZE_FILENAME, FILE_PREFIX_CLIENT, getpid(), FILE_SUFFIX_FIFO) == -1) {
        so_error("C11", "");
    } else {
 ...
 *              via Shell), o Cliente dá so_success "Shutdown Cliente", e depois, abre o FIFO do Servidor Dedicado, escreve nesse
...
void trataSinalSIGINT_C13(int sinalRecebido) {
    ...
    if (buildNomeFifo(nameFifoServidorDedicado, SIZE_FILENAME, FILE_PREFIX_SRVDED, clientRequest.pid_servidor_dedicado, FILE_SUFFIX_FIFO) == -1) {
        so_error("C13", "");
    } else {
        FILE* f = fopen(nameFifoServidorDedicado, "w");
        if (f) {
            if (fprintf(f, "Sessão cancelada pelo utilizador") < 0)
                so_error("C13", "Failed to write to file %s", nameFifoServidorDedicado);
            else so_success("C13", "");
            fclose(f);
        } else
            so_error("C13", "Failed to open file %s", nameFifoServidorDedicado);
