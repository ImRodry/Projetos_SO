...
#define EVAL_CATCH( _code ) { \
    ...
    _eval_env.signal = -1; \
    _eval_arm_signals(); \
    fprintf(stderr, "\033[1;33m ⊢ \033[0m %s running...\n", #_code ); \
    _eval_env.stat = sigsetjmp( _eval_env.jmp, 1 ); \
    if ( !_eval_env.stat ) { \
        _code; \
        fprintf(stderr, "\033[1;33m ⊣ \033[0m %s completed normally.\n", #_code); \
    } else { \
        switch( _eval_env.stat ) { \
        case(1): \
            fprintf(stderr, "\033[1;33m ⊣ \033[0m %s terminated by exit(%d)\n", #_code, _eval_exit_data.status ); \
            break; \
        case(2): \
            if ( _eval_env.signal == SIGPROF ) { \
                fprintf(stderr, "\033[1;33m ⊣ \033[0m %s timeout after %g second(s)\n", #_code, _eval_env.timeout);\
            } else { \
                fprintf(stderr, "\033[1;33m ⊣ \033[0m %s terminated by signal %d\n", #_code, _eval_env.signal);\
            } \
            break; \
        default: \
