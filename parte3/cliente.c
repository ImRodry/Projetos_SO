...
int triggerSignals_C2() {
    ...
    result =
        signal(SIGUSR2, trataSinalSIGUSR2_C10) == SIG_ERR ||
        signal(SIGINT, trataSinalSIGINT_C11) == SIG_ERR ||
        signal(SIGALRM, trataSinalSIGALRM_C12) == SIG_ERR ? RET_ERROR : RET_SUCCESS;
...
MsgContent getDadosPedidoUtilizador_C3() {
    ...
    msg.msgData.infoLogin.nif = atoi(s);

    printf("Senha: ");
    if (!so_gets(s, sizeof(msg.msgData.infoLogin.senha))) { // Should return the pointer to the string or null
        so_error("C3", "Failed to read password");
        msg.msgData.infoLogin.nif = -1;
        return msg;
    }
    strcpy(msg.msgData.infoLogin.senha, s);
...
int receiveProductList_C6() {
    ...
    so_success("C6", "%s %d %d", msg.msgData.infoLogin.nome,
        msg.msgData.infoLogin.saldo, msg.msgData.infoLogin.pidServidorDedicado);
    ...
    while (msg.msgData.infoProduto.idProduto != FIM_LISTA_PRODUTOS) {
        printf("ID: %d, Nome: %s, Categoria: %s, Preço: %d\n", msg.msgData.infoProduto.idProduto,
            msg.msgData.infoProduto.nomeProduto, msg.msgData.infoProduto.categoria,
            msg.msgData.infoProduto.preco);
        if (msgReceive(msgId, &msg, getpid()) == -1) {
            so_error("C6.5", "");
            return result;
        }
...
int getIdProdutoUtilizador_C7() {
    ...
    printf("Indique o ID do produto que deseja adquirir: ");
    result = so_geti();
    if (result <= 0) {
        so_error("C7", "");
...
void receivePurchaseAck_C9() {
    ...
    if (msgReceive(msgId, &msg, getpid()) == -1)
        ...
        else printf("Ocorreu um problema na sua compra. Tente novamente");
    }
