...
int initShm_S1() {
    ...
    shmId = shmCreate(1); // 1 * sizeof DadosServidor which has the necessary data
    if (shmId < 0) {
        so_error("S1.3", "");
        return result;
    }
    ...
    FILE* fileUsers = fopen(FILE_DATABASE_USERS, "r");
    if (!fileUsers) {
        so_error("S1.5", "");
        return result;
    }
    fread(db->listUsers, sizeof(Login), MAX_USERS, fileUsers); // There can be less than MAX_USERS, so no need to check for error
    if (ferror(fileUsers)) {
        so_error("S1.5", "");
        return result;
    }
    ...
    for (int i = 0; i < MAX_USERS; i++) {
        // Sets them to -1
        db->listUsers[i].pidCliente = USER_NOT_FOUND;
        db->listUsers[i].pidServidorDedicado = USER_NOT_FOUND;
    }
    so_success("S1.5", "");

    FILE* fileProducts = fopen(FILE_DATABASE_PRODUCTS, "r");
    if (!fileProducts) {
        so_error("S1.6", "");
        return result;
    }
    fread(db->listProducts, sizeof(Produto), MAX_PRODUCTS, fileProducts); // Same as above for MAX_PRODUCTS
    if (ferror(fileProducts)) {
        so_error("SD1.6", "");
        return result;
    }
...
int initMsg_S2() {
    ...
    if (msgRemove(msgGet()) == 0) // If it was removed successfully or if it didn't exist
        so_success("S2.1", "");
    else {
        so_error("S2.1", "");
        return result;
...
int initSem_S3() {
    ...
    semId = semGet();
    if (semId == -1 || semRemove(semId) == 0) // If it was removed successfully or if it didn't exist
        so_success("S3.1", "");
    else {
        so_error("S3.1", "");
        return result;
    ...
    if (
        semNrSetValue(semId, SEM_USERS, SEM_MUTEX_INITIAL_VALUE) < 0 ||
        semNrSetValue(semId, SEM_PRODUCTS, SEM_MUTEX_INITIAL_VALUE) < 0 ||
        semNrSetValue(semId, SEM_NR_SRV_DEDICADOS, 0) < 0
        ) {
        so_error("S3.3", "");
        return result;
    }
...
int triggerSignals_S4() {
    ...
    result = signal(SIGINT, handleSignalSIGINT_S8) == SIG_ERR ||
        signal(SIGCHLD, handleSignalSIGCHLD_S9) == SIG_ERR ? RET_ERROR : RET_SUCCESS;
...
MsgContent receiveClientLogin_S5() {
    ...
    if (msgReceive(msgId, &msg, MSGTYPE_LOGIN) == RET_ERROR) {
        if (errno != EINTR) so_error("S5", "");
        else so_debug("msgReceive was interrupted by a system signal. That's fine though, will continue listening for a message...");
        msg.msgData.infoLogin.nif = USER_NOT_FOUND; // Make sure it gets returned with error
        return msg;
    }
...
int createServidorDedicado_S6() {
    ...
    if (pid_filho == -1)
        so_error("S6", "");
    else if (pid_filho != PROCESSO_FILHO) // Only the parent sends the success message
...
void shutdownAndExit_S7() {
    ...
    if (shmId < 0 && db != NULL)
        ...
        so_success("S7.1", "");
        for (int i = 0; i < MAX_USERS; i++)
            if (db->listUsers[i].pidServidorDedicado != USER_NOT_FOUND)
        ...
        if (f == NULL)
            ...
            for (int i = 0; i < MAX_PRODUCTS; i++) {
                if (db->listProducts[i].stock > 0)
                    if (fwrite(&db->listProducts[i], sizeof(Produto), 1, f) < 1) {
                        result = -1;
                        so_error("S7.4", "");
                        break;
                    }
    ...
    if (shmRemove(shmId) == RET_ERROR || semRemove(semId) == RET_ERROR || msgRemove(msgId) == RET_ERROR)
        so_error("S7.5", "");
    else
        so_success("S7.5", "");
...
void handleSignalSIGCHLD_S9(int sinalRecebido) {
    ...
    so_success("S9", "Terminou Servidor Dedicado %d", childPid);
...
int triggerSignals_SD10() {
    ...
    result = signal(SIGUSR1, handleSignalSIGUSR1_SD18) == SIG_ERR ||
        signal(SIGINT, SIG_IGN) == SIG_ERR ? RET_ERROR : RET_SUCCESS;
    ...
    for (int i = 0; i < MAX_USERS; i++) {
        if (db->listUsers[i].nif == request.nif) {
            ...
            if (strcmp(db->listUsers[i].senha, request.senha) == 0) {
                ...
                break;
            } else so_error("SD12.1", "");
        }
    }
    if (!found) so_error("SD12.1", "");
    if (indexClient == USER_NOT_FOUND) {
        msg.msgData.infoLogin.pidServidorDedicado = -1;
        if (msgSend(msgId, msg, request.pidCliente) < 0)
            so_error("SD12.2", "");
        else so_success("SD12.2", "");
    }
...
MsgContent receiveClientOrder_SD15() {
    ...
    if (msgReceive(msgId, &msg, getpid()) == RET_ERROR || msg.msgData.infoLogin.nif == USER_NOT_FOUND) {
        so_error("SD15", "");
        return msg;
    }
    so_success("SD15", "");
...
int sendPurchaseAck_SD16(int idProduto, int pidCliente) {
    ...
    if (idProduto == PRODUCT_NOT_FOUND) {
        ...
        if (!productIndex || db->listProducts[productIndex].stock <= 0) {
            msg.msgData.infoProduto.idProduto = PRODUTO_NAO_COMPRADO;
            so_error("SD16.2", "");
        } else {
            db->listProducts[productIndex].stock--;
    ...
    if (msgSend(msgId, msg, pidCliente) == -1)
        so_error("SD16.3", "");
    else {
        so_success("SD16.3", "");
        result = RET_SUCCESS;
